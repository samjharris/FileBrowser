...
class MachineDoc:
    def __init__(self, db, ssn, model, tenants=["HPE"]):
        ...
        self.last_update = datetime.datetime.strptime("1900", "%Y")
        self.logs = []
        self.data = {
            "ssn": self.ssn,
            "model": self.model,
    ...
    def update_db(self, validator=None):
        ...
        if doc:
            ...
            #
            update = coll.find_one_and_update(
                {"ssn": self.ssn},
                {
                    "$set": {
    ...
    def update_log_links(self, docs=None, strays=False):
        ...
        """
        m_coll = self.db["machines"]
        l_coll = self.db["logs"]
        # get the _id of one existing log for this machine
        doc = m_coll.find_one({"ssn": self.ssn})
        ...
        if strays:
            if not gen:
                ...
                cur = l_coll.find({"serialNumberInserv": gen["serialNumberInserv"]})
                for res in cur:
                    res_id = res["_id"]
                    red_date = res["date"]
        ...
        if docs:
            for log_id in docs:
                m_coll.find_one_and_update(
                    {"ssn": self.ssn},
                    {"$set": {"logs": doc["logs"] + list(set([log_id]) - set(doc["logs"]))}},
                )
                self.update_date(l_coll.find_one({"_id": log_id}))
    ...
    def update_date(self, log):
        """
        Updates the most recent date that a log was inserted into this machine
        """

        insertion_date = self.convertDate(log["date"])
        coll = self.db["machines"]
        update_doc = coll.find_one({"ssn": self.ssn})
        if not update_doc:
            return False
        compare_date = update_doc["last_update"]
        if insertion_date > compare_date:
            coll.find_one_and_update({"ssn": self.ssn}, {"$set": {"last_update": insertion_date}})
        return True

    def convertDate(self, date_string):
        """
        Converts a date from the string in one of our JSON files to a Python datetime object
        """
        log_year = re.search("^....", date_string).group(0)
        log_month = re.search("-..-", date_string).group(0)[1:3]
        log_day = re.search("-[0-9]+T", date_string).group(0)[1:3]
        log_hour = re.search("T.+:", date_string).group(0)[1:3]
        log_minute = re.search("T.+:.+:", date_string).group(0)[4:6]
        log_second = re.search("T.+:.+:.+Z", date_string).group(0)[7:9]
